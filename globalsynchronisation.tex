...
process calculi, such as those described in \cite{aceto:timing,
  beaten:timing, brics:lee, lee:realtime, tccs, satoh:phd,
  satoh:distrib}, attempt to model this.  Instead, this section
focuses on calculi that use \emph{clocks} to provide global
synchronisation, as introduced in \ref{ccslimit}.  While \emph{local
  synchronisation} occurs between two processes, the global form
synchronises any number of processes using a clock signal.
