...
transition was derived, \texttt{t} and \texttt{t2}, are also stored.
...
The channel framework is accessed through four subclasses of
\texttt{Action}, three of which are also used in \texttt{Prefix}, the
implementation of $\alpha.E$:
...
\item The \texttt{Sync} class is created through \texttt{Par} (see
  \ref{dyn:maptheory}) and implements \texttt{perform()} using the two
  synchronising actions provided to it on construction.
\end{enumerate}
