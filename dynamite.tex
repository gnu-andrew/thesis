...
chapter \ref{nt} as a foundation for application development.  When
using DynamiTE, programmers compose NT processes, realised as Java
objects, to create a working system.  The framework handles running
these processes, in parallel if necessary, and negotiates the
...
The first half of this chapter explores the design of the framework
and its initial implementation in the Java programming language. In
\ref{dyn:maptheory}, we describe how Nomadic Time processes are mapped
onto Java objects (see \ref{dyn:maptheory}) with the provision of a
translation schema (\cont{2.1}) and examples of how the semantics are
implemented (\cont{2.2}).  The next section, \ref{dyn:context}, deals
with practicalities beyond simply implementing the syntax of the
calculus; we describe how the \emph{context} of the calculus is
represented and also how we attach additional side effects to
transitions via the plugin framework (\cont{2.3}).  In
\ref{dyn:evolvers}, we look at the final piece of the jigsaw;
executing the processes in order to produce behaviour.  The execution
semantics are embodied in the evolver framework (\cont{2.4}), which
allows the programmer to swap in different implementations, depending
on whether they want to simply examine all the possible transitions of
a process or to follow a single deterministic route with side effects.

Finally, we show how DynamiTE can be used to create an implementation
of the application we introduced in \ref{app:req} (see
\ref{app:dynamite}), before closing with a brief look at other similar
concurrent frameworks in \ref{dyn:relatedwork}.  But first, we discuss
why we chose Java as the host language for DynamiTE and what
...
\item \textbf{All memory management is performed by a garbage
  ...
  the application.  Java instead makes use of existing research into
  \emph{garbage collectors} which allocate memory as needed and
  periodically reclaim it.  The downside of this is that the garbage
  collector has to use processor time to perform its scans which would
  otherwise be used by the application.  However, as new garbage
  collection techniques, such as concurrent and generational
  collectors, become prevalent, this disadvantage is further
  outweighed by the prospect of chasing memory leaks.
...
programmer and possible efficiency gains, they save time overall in
...
seems to have no advantage, other than simplifying the language.  Many
...
machine rather than as an auxiliary library (the approach used for C).
...
Popularity is rarely a good reason to do anything but, in combating
...
A rough translation schema is given in \ref{tab:translationschema1},
...
further instances as appropriate.  Table \ref{tab:translationschema2}
supplements the schema, by showing how names, co-names, silent actions
and clocks are converted into objects.  The first three are all
subclasses of \texttt{Action}, more details of which can be found in
\ref{dyn:evolvers}.

\begin{table}
 \caption{Translation Schema from NT to DynamiTE \texttt{Process} Subclasses}
 \label{tab:translationschema1}
  \shrule \centering
  \begin{tabular}{rcl}
...
\begin{table}
 \caption{Translation Schema from NT to DynamiTE Classes}
 \label{tab:translationschema2}
  \shrule \centering
  \begin{tabular}{rcl}
    $\seml a \semr$ & = & \texttt{new Name("a")} \\
    $\seml \overline{a} \semr$ & = & \texttt{new Coname("a")} \\
    $\seml \tau \semr$ & = & \emph{Subclass of \texttt{Tau}} \\
    $\seml \sigma \semr$ & = & \texttt{new Clock("sigma")} \\
  \end{tabular}
  \shrule
\end{table}
...
meet the prerequisites for one of the rules and then creating new
...
used with co-names; these are \texttt{convertLabelToName},
\texttt{convertConameToLabel} and \texttt{isConame}, each of which
take a single \texttt{String} argument.  In our implementation, we
differentiate names from co-names by marking each letter with a
combining macron (so $a$ becomes $\overline{a}$).
...
a constructor.  The user calls \texttt{ContextFactory.getInstance},
supplying three \texttt{String} arguments; the name of a process
calculus, a \emph{channel implementation} and a \emph{locality
  implementation}.  The two latter arguments are used to determine the
execution semantics for synchronisation and movement respectively.
The returned \texttt{ContextFactory} will be able to supply a
\texttt{Context} instance for the specified process calculus which
uses the given channel and locality implementations.  If one can not
be found, an \texttt{UnsupportedContextException} is thrown.
...
return a \texttt{Context} instance.  This is stored using the static
method \linebreak \texttt{Context.setContext(Context)}, and
later retrieved by \texttt{Context.getContext()}.
...
extend \texttt{Probeable} so \linebreak
\texttt{DynamiTEContextFactory} need only call the above method with
an appropriate service loader for the interface and it will receive
back a \texttt{Map} linking names to instances of implementations of
that interface.  Most of the actual work is done by
\texttt{ServiceLoader} which reads from a text file named after the
interface, which contains a list of implementing classes.  The
DynamiTE framework supplies such text files for its implementations of
\texttt{ContextFactory}, \texttt{Calculus}, \texttt{ChannelFactory}
and \texttt{LocalityFactory}.  The \texttt{ServiceLoader} loads a
listed class each time its \texttt{next()} method is
called\footnote{This happens indirectly in \texttt{probePlugins} via
  each iteration of the for-each loop.}, and returns an instance of
it, which \texttt{probePlugins} then stores in the map.
...
A more practical \texttt{Evolver} is a more complex undertaking;
further research in this area is suggested in \ref{future:dynamite}.
It has to make choices as to which transition to pick when several are
...
\item Likewise, the class \texttt{Coname} represents the use of a
  ...
  retrieving a value from storage and transmitting it over an \linebreak
  \texttt{OutputChannel}.
...
obtained from the \linebreak \texttt{ChannelFactory}, via the \texttt{Context},
...
\linebreak \texttt{ThreadLocal} instance has been created for a
particular channel name, and if not, create one.  The initial
\texttt{get} operation will return \texttt{null} if we have not yet
created a mapping between that channel name and a
\texttt{ThreadLocal}.  However, in the time it takes for this thread
to check the return value and create a \texttt{ThreadLocal}, another
thread may have added such a mapping.  The \texttt{ConcurrentMap}
interface (which \texttt{ConcurrentHashMap} implements) provides an
additional operation for just this issue: \texttt{putIfAbsent}.  This
provides an atomic version of the following code:
...
for future work (see \ref{future:dynamite}).
...
a number of possible solutions to this.  For example:
...
output; although it still decomposes nicely, we have to handle
synchronisation ourselves.  We dispense with the
\texttt{BlockingQueue} as we need to obtain a lock anyway to ensure
the atomicity of the multiple \texttt{offer} calls in \texttt{input}
and the checks in \texttt{output} and \texttt{visualise}.  Both the
\texttt{output} and \texttt{visualise} methods are very similar and it
would be a very good idea to generalise these in a \texttt{Consumer}
superclass; as we noted in \ref{semaphores}, the correct placement of
locking and signalling constructs is prone to simple errors so
anything we can do to simplify this and minimise the risk is
advantageous.
...
such as \texttt{store} and \texttt{retrieve} calls to the
repositories.  In implementing DynamiTE, we believe we have also made
it approachable for existing Java programmers, both for building their
own applications and for contributing to DynamiTE itself.  By learning
...
primarily due to its status as the most prevalent mobile process
calculus.  Obliq \cite*{obliq} and Pict \cite*{daveturner:phd} are
both programming languages with semantics founded in the $\pi$
calculus, while Nomadic Pict \cite*{wojciechowski:phd} extends Pict by
introducing distribution, a feature not usually present in the $\pi$
calculus.  The Seal calculus \cite*{seal} is an example of yet another
distributed variant of the $\pi$ calculus, and this has also lead to
an implementation in the form of JavaSeal \cite*{javaseal}.  However,
the ambient calculus has not been neglected, and an implementation
exists in the form of the safe ambients abstract machine
\cite*{sangiorgi:safeambientsmachine}.  In the remainder of this
chapter, we look at these implementations in more detail.
...
and is fairly inaccessible for those who don't know the calculus in
...
very useful in some areas, it is not suitable as a general purpose
language for the masses.

\subsection{The Safe Ambients Abstract Machine}
\label{safeambmachine}
...
Through our example in \ref{app:dynamite}, we saw how DynamiTE
achieves its goal of simplifying the creation of concurrent
applications by abstracting away many complex and fragile pieces of
code which ensure thread safety, replacing them with simple abstract
concepts such as \texttt{store} and \texttt{retrieve} calls to the
repositories.  By learning the meaning of a small number of algebraic
concepts, programmers can leverage the power of DynamiTE to create
applications limited only by the plugin factories being used.

In this chapter, we have presented the overall structure of the
DynamiTE framework for concurrent systems, giving the second set of
novel contributions (\cont{2.1} through \cont{2.4}) in this thesis.
This includes the translation from Nomadic Time to a process framework
(\cont{2.1}) with an implementation of the operational semantics
(\cont{2.2}), the plugin framework for introducing side effects
(\cont{2.3}) and the evolver framework for working with execution
semantics (\cont{2.4}).  We believe that the framework provides a
...
\ref{globsync} and mobility.  The existence of a formal theory for
...
for existing Java programmers, both for building their own
applications and for contributing to DynamiTE itself.  The actual
implementation of the DynamiTE framework is still in heavy
...
leveraged by the programmer in the same simple manner provided by the
framework.

In the next chapter, we look at how Nomadic Time, and DynamiTE in
turn, can be extended with a type system, providing the final set
(\cont{C3.1} to \cont{C3.4}) of novel contributions.  We begin the
chapter with a look at existing work on type systems for process
calculi in \ref{typedcalculi}, before moving onto the design of our
typing rules (\cont{3.2}) using a group type for processes
(\cont{3.1}) to define further restrictions on migration.  The
standard type safety proofs of progress and progression are given
(\cont{3.3}), before we again return to DynamiTE in \ref{dyn:type} to
show how it may be extended to support this new type system
(\cont{3.4}).
