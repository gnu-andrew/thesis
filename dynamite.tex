...
(\texttt{t.getFinish()}).  Thus, all the new transitions returned use
the same label (\texttt{t.getLabel()}), the current instance of
\texttt{Rec} as the start state and a version of the original final
state with substitution applied.
...
the start state, the same transition label as the original
(\texttt{t.getLabel()} and a new \texttt{Par} instance as the final
...
state, the \texttt{CCSLabel.TAU} singleton as the label and a final
state created by joining together the two final states from each
transition in a new \texttt{Par} instance.
...
their additional rules.  The other constructs, such as $+$, the
