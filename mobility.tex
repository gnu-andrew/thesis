...
moved up and down a nested hierarchy of such objects, or be destroyed.
The calculus differs from those previously considered, in that it
...
The $\pi$ calculus \cite*{milner:pi} follows on from Milner's earlier
...
two processes synchronise, the system evolves to:
...
(second-order), but also parametrised processes of arbitrarily high
...
HO$\pi$, the parametrised processes or \emph{agents} that represent
...
\subsection{Advantages and Limitations of the $\pi$ Calculus}
\label{pilimit}
...
How this actually works in practise is best shown by reconsidering the
...
For the first transition, both are capable of performing
$\locderives{l}{in}$ and can thus match each other.  However, the
relation breaks down on the second transition which compares
$\locderives{lk}{\overline{out}}$ with
$\locderives{l}{\overline{out}}$.  Under a normal weak bisimulation,
these two transitions would be judged equivalent, as only the action
is available for comparison; both perform an $\overline{out}$.
However, a localised bisimulation requires the locations to also
match, which fails here.  The specification has a longer causal path,
as its single process has performed two visible actions.  In contrast,
the two processes involved in the implementation have performed one
action each, resulting in two separate paths with a length of one.
...
\linebreak $\ambin{n}.\ambout{n}.P$ begins its life in the ambient $m$
...
for $a$ to synchronise with.  This behaviour is particularly relevant
...
synchronise with a co-capability in the relevant ambient for the
...
directed at the parent ambient.  Thus, $(x)Q$ may synchronise with
either $\langle M \rangle$ locally or the upward communication from
$\langle N \rangle$.  $(x)^pP$ must synchronise with $\langle M
...
\subsection{Advantages and Limitations of the Ambient Calculus}
\label{amblimit}
...
problem of enumerating the number of entities who wish to synchronise.
...
synchronise, as \texttt{Mol} is not inside the ambient from which it is
